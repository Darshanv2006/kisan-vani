...
def get_call_analytics():
    ...
    cursor.execute(
        "SELECT COUNT(*) as total FROM call_analytics WHERE status = 'SUCCESS'"
    )
    successful_calls = cursor.fetchone()["total"]

    cursor.execute(
        "SELECT COUNT(*) as total FROM call_analytics WHERE status = 'FAILED'"
    )
    ...
    # Recent 20 calls
    cursor.execute("SELECT * FROM call_analytics ORDER BY created_at DESC LIMIT 20")
