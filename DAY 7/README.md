...
## Day 5 — Adding Domain Tools (Kisan Vani)

In Day 5, Kisan Vani is enhanced with domain tools that enable the agent to fetch real-time agricultural data off the internet:

### Tools Implemented
1. **`get_weather_forecast(district)`**:
   - **Data Source**: **Live Open-Meteo API** (Real-time public REST service, no API key required).
   - **Data Returned**: Current temperature (°C), max/min daily temperature, relative humidity (%), wind speed (km/h), and max rain probability (%) with `as_of_date` timestamps.
2. **`get_mandi_market_prices(crop_name, district)`**:
   - **Data Source**: Real APMC Mandi market rates per quintal in ₹ (INR) for Indian crops (Cotton, Wheat, Rice, Mustard, Potato, Onion) across major markets (Bhatinda, Ludhiana, Karnal, Nashik, Jaipur, etc.) tagged with live timestamps (`as_of_date`).

### Failure & Error Handling
- **Graceful Spoken Fallback**: Both tools feature strict 5-second timeouts and try-catch error boundaries. If an API times out or a crop/district is unavailable, the tool returns a clean error dictionary. The agent gracefully explains the issue out loud in natural Devanagari Hindi or English, avoiding silent drops or hallucinations.
