...
async def main():
    parser = argparse.ArgumentParser(description="Make an outbound call to Linphone / SIP via LiveKit Telephony.")
    parser.add_argument(
        "--to",
        required=True,
    ...
    trunk_id = args.trunk
    if not trunk_id:
        print("❌ Error: LIVEKIT_SIP_OUTBOUND_TRUNK_ID environment variable or --trunk flag is missing!")
        print("Please set LIVEKIT_SIP_OUTBOUND_TRUNK_ID in backend/.env.local or pass --trunk ST_xxxx")
        sys.exit(1)
