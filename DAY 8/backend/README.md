...
## Day 5 — Domain Tools Integration

Kisan Vani incorporates real-time data lookup tools via LiveKit `@function_tool` decorators:

### Tools Added
- **`get_weather_forecast(district)`**: Uses the **Open-Meteo REST API** (live, free public service) to fetch real-time temperature, max/min range, wind speed, relative humidity, and precipitation probability for Indian districts.
- **`get_mandi_market_prices(crop_name, district)`**: Queries APMC Mandi market rates per quintal in ₹ (INR) for Indian crops (Wheat, Cotton, Rice, Mustard, Potato, Onion) with explicit `as_of_date` timestamps.

### Testing Tools
Run the Day 5 unit test suite:
```bash
uv run python -m pytest tests/test_day5_tools.py
```
